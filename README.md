...
- модели SQLAlchemy `SourceChannel`, `TelegramPost`, `Episode` и `EpisodePost`;
- SQLite-хранилище;
...
`make podcast-publish` не генерирует выпуск заново, а отправляет последний готовый
пакет из `data/episodes`. Если нужно собрать новый выпуск и сразу отправить:
...
Сценарий собирается в несколько шагов:

1. `script_draft.md` — фактический черновик из выбранных Telegram-постов.
2. LLM-сценарист превращает новости в диалог четырёх ведущих.
3. LLM-редактор делает разговор живее: убирает роботичные повторы, добавляет
   реакции, разные переходы между темами и нормальное прощание.
4. Валидатор и постобработка проверяют формат для TTS: только строки
   `mark:`, `nika:`, `gleb:`, `artem:`, без служебного текста и сломанных
   обращений.

Если редакторская версия получается невалидной, проект автоматически берёт
предыдущий валидный сценарий. Это защищает длинную генерацию аудио от случайного
срыва на этапе текста.
...
Проверить выпуск перед озвучкой:

```bash
make podcast-check
```

Команда показывает выбранные темы, наличие файлов, качество сценария, блокеры
перед TTS, предупреждения и риск слишком длинных реплик.
...
При автоматическом выборе тем пайплайн исключает посты из последнего
завершённого выпуска. Основная история выпусков хранится в SQLite-таблицах
`episodes` и `episode_posts`; старые файловые пакеты с `selected_posts.json`
используются как резервный источник. Так одна и та же новость не попадёт в
следующий подкаст повторно.
...
диалог, затем редактор-шоураннер делает разговор живее, убирает механические
повторы, добавляет реакции персонажей и проверяет финальное прощание. Если
редакторская версия ломает формат, проект автоматически откатывается к
валидному сценарию. Отключить второй проход можно так:
...
```text
app/llm/prompts/dialogue_scriptwriter.md
app/llm/prompts/dialogue_editor.md
```

Первый отвечает за структуру выпуска и факты, второй — за живость сцены,
разнообразие реплик, переходы между темами и человеческий финал.
...
Проверить качество и параметры референсов:

```bash
make voice-report
```

Для XTTS лучше держать чистые WAV-референсы примерно на 6-20 секунд, без музыки,
шума и чужих голосов. Отчёт покажет длительность, частоту дискретизации, каналы
и предупреждения по каждому персонажу.
